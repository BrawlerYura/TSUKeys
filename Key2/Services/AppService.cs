...
namespace Key2.Services
{
    public class AppService:IAppService
    {
        ...
        async Task<Guid> IAppService.CreateApp(CreateAppModel model, Guid userId)
        {
            ...
            if ((user.IsDeanWorker == true && user.DeanId == key.DeanId))
            {
        ...
        async Task<List<Application>> IAppService.AllApps(Guid userId)
        {
            List<Application> apps = await _context.apps.Include(k => k.Key).Where(app => app.AppFromUser.Id == userId && app.Date >= DateTime.Today).ToListAsync();
            return apps;
        }
    }
}
