...
namespace Key2.Services
{
    public class DeanService : IDeanService
    {
        ...
        async Task<DeanResponseModel> IDeanService.LoginBothAsync(LoginCredentialsModel LC)
        {
            var dean = await _context.deans.SingleOrDefaultAsync(u => u.Email == LC.Email);
            ...
            if (dean == null || HashUserPassword != dean.Password)
            {
                if (user == null || HashUserPassword != user.Password)
                {
                    throw new ArgumentException("Bad Data");
                }
                else if(user.IsActive != true || user.IsDeanWorker != true)
                {
                    DeanResponseModel emptyToken = new DeanResponseModel();
                    emptyToken.Token = "";
                    return emptyToken;
                }
                else
                {
                    var newDean = await _context.deans.FindAsync(user.DeanId);
                    TokenResponseModel deanToken = GenerateToken(newDean);
                    TokenResponseModel userToken = GenerateToken(user);
                    DeanResponseModel deanResponseModel = new DeanResponseModel() { Token = deanToken.Token, UserToken = userToken.Token};
                    return deanResponseModel;
                }
            }
            if (dean.IsActive != true)
            {
                DeanResponseModel emptyToken = new DeanResponseModel();
                emptyToken.Token = "";
                return emptyToken;
            }
            TokenResponseModel token = GenerateToken(dean);
            return new DeanResponseModel() { Token = token.Token};
        }

        async Task IDeanService.ReturnKey(Guid id)
        {
        ...
        async Task IDeanService.HandOver(Guid id, Guid userId)
        {
            ...
            var key = await _context.keys.SingleOrDefaultAsync(k => k.Id == id);
            if (key == null)
            {
                throw new DirectoryNotFoundException("KeyNotFound");
            }
