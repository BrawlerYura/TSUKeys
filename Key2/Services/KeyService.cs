...
namespace Key2.Services
{
    public class KeyService : IKeyService
    {
        ...
        async Task<Guid> IKeyService.CreateKey(int Number, Guid userId, int building = 0)
        {
            ...
            var key = await _context.keys.FirstOrDefaultAsync(d => d.Number == Number && d.Building == building);
            if(key != null)
            {
        ...
        {
            List<KeySchedulePreview> lst = await _context.apps.Include(a => a.Key).Where(a => a.Key.Id == Id && a.IsConfirmation == true).Select(item => new KeySchedulePreview { Date = item.Date, Sheduller = item.Schedule, UserId = item.AppFromUser.Id, UserName = item.AppFromUser.Name, AppId = item.Id }).ToListAsync();
            return lst;
        }
