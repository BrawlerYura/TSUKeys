...
namespace Key2.Services
{
    public class UserService : IUserService
    {
        ...
        async Task<TokenResponseModel> IUserService.RegisterAsync(RegisterModel RegisterModel)
        {
            ...
            if(!Regex.IsMatch(RegisterModel.Email, RegexEmail))
            {
                throw new ArgumentException("Bad Email");
            }
                if (!Regex.IsMatch(RegisterModel.BirthDate.ToString(), RegexBirthDate) || RegisterModel.BirthDate.Value.Year >= DateTime.Now.Year)
            {
        ...
        async Task<UserProfileModel> IUserService.GetProfileAsync(Guid userId)
        {
            ...
            UserProfileModel model = new UserProfileModel { Email = user.Email, Name = user.Name, BirthDate = user.BirthDate, IsActive = user.IsActive, Role = user.Role, IsDeanWorker = user.IsDeanWorker, IsDenied = user.IsDenied };
            return model;
        }

        async Task IUserService.EditProfileAsync(EditUserInfoModel EditModel, Guid userId)
        {
            ...
            var HashUserPassword = EditModel.Password != "" ? HashPassword(EditModel.Password) : user.Password;
        ...
        */
        ...
        async Task IUserService.ReadQR(Guid userId, string pass)
        {
            var user = await _context.mobileUsers.SingleOrDefaultAsync(u => u.Id == userId);
            var qr = await _context.QRPass.FirstOrDefaultAsync(q => q.QRPas == HashPassword(pass));
            if (qr == null)
            {
                throw new DirectoryNotFoundException("QR not found");
            }
            var key = await _context.keys.SingleOrDefaultAsync(k => k.Id == qr.keyId);
            if (key != null)
            {
                key.CurrentUser = user;
                key.CurrentUserId = userId;
