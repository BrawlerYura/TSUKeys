...
namespace Key2.Controllers
{
    ...
    public class DeanController : ControllerBase
    {
        ...
        [Route("loginBoth")]
        [HttpPost]
        public async Task<IActionResult> LoginBoth([FromBody] LoginCredentialsModel LC)
        {
            try
            {
                var result = await _deanService.LoginBothAsync(LC);
                return Ok(result);
            }
            catch (ArgumentException ex)
            {
        ...
        [Route("returnKey")]
        [HttpPost]
        public async Task<IActionResult> ReturnKey(Guid id)
        {
            try
            {
                await _deanService.ReturnKey(id);
                return Ok();
            }
            catch (ArgumentException ex)
            {
        ...
        [Route("handOver")]
        [HttpPost]
        public async Task<IActionResult> HandOver(Guid id, Guid userID)
        {
            try
            {
                await _deanService.HandOver(id, userID);
                return Ok();
            }
            catch (ArgumentException ex)
            {
