...
namespace Key2.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPut]
        [Authorize]
        public async Task<IActionResult> EditProfile([FromBody] EditUserInfoModel userEditModel)
        {
            try
            {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateChangeRole(Role desiredRole, Guid deanId)
        {
            try
            {
                TokenBan token = new TokenBan { BannedToken = HttpContext.Request.Headers["Authorization"].ToString().Replace("Bearer ", "") };
        ...
        [Route("createQR")]
        [HttpPost]
        public async Task<IActionResult> CreateQR(Guid keyId, string pass)
        {
            try
            {
                TokenBan token = new TokenBan { BannedToken = HttpContext.Request.Headers["Authorization"].ToString().Replace("Bearer ", "") };
        ...
        [Route("readQR")]
        [HttpPut]
        public async Task<IActionResult> ReadQR(string pass)
        {
            try
            {
                TokenBan token = new TokenBan { BannedToken = HttpContext.Request.Headers["Authorization"].ToString().Replace("Bearer ", "") };
                _banTokensService.CheckAuthentication(token);
                Guid userId = Guid.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier));
                await _userService.ReadQR(userId, pass);
                return StatusCode(200, "OK");
            }
            catch (DirectoryNotFoundException ex)
            {
        ...
        [Route("deleteQr")]
        [HttpDelete]
        public async Task<IActionResult> DeleteQR(Guid keyId)
        {
            try
            {
                TokenBan token = new TokenBan { BannedToken = HttpContext.Request.Headers["Authorization"].ToString().Replace("Bearer ", "") };
